# 海風 SeaWind 網站設計文件
| 2026.05.14 | 海風網站助手 | **贊助頁季卡定位重調整＋Hero 潤飾＋移除資金用途＋移除贈卡＋對比表欄位重排**：①季卡卡片：從「9 項功能，到期自動關閉」改為主打「活動＋季節額外獎勵」，隨地開啟降為附贈功能；②會員卡片：「22 項永久回饋」→「VIP 永久身份」；③組合卡片：「一次解鎖全部 31 項回饋」→「一次解鎖 VIP 永久身份＋當季活動加成」；④對比表欄位順序從「預設/會員/季卡」→「預設/季卡/會員」（由低到高）；⑤對比表：季卡專屬區移至待設計前，隨地開啟標籤→「兩個方案都有」；⑥FAQ 重寫——會員vs季卡差別改為「長期投資 vs 活動季加成」；⑦meta/og description 同步更新；⑧Hero 文案潤飾；⑨移除資金用途區塊；⑩移除贈卡區塊＋贈卡FAQ。cache busting → 140514f。頁尾日期 → 2026.05.14 |
