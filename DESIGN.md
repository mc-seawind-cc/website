# 海風 SeaWind 網站設計文件
| 2026.05.14 | 海風網站助手 | **贊助頁季卡定位重調整＋區塊順序重排**：①季卡卡片主打「活動＋季節額外獎勵」，隨地開啟降為附贈；②會員卡片→「VIP 永久身份」；③組合卡片→「VIP 永久身份＋當季活動加成」；④對比表欄位順序「預設/季卡/會員」（由低到高）；⑤對比表季卡專屬區移至待設計前，隨地開啟→「兩個方案都有」；⑥FAQ 重寫；⑦移除資金用途＋贈卡區塊；⑧Hero 文案潤飾；⑨區塊順序重排：Hero→回饋總覽→價格卡→注意事項→FAQ→贊助流程→相關頁面。cache busting → 140514f。頁尾日期 → 2026.05.14 |
