...
### 歷史館設計（v5 — 2026.04.14 重構）

**設計理念：** 歷史應該一目了然，不需要點來點去。

**架構：**
- 所有事件直接顯示在頁面上，無需點擊展開
- 時間線樣式保留（左側圓點 + 連接線），里程碑事件以卡片式突出
- 頂部僅有三個時代切換鈕：全部時期 / 純麥時期 / 海風時期
- 移除標籤篩選（里程碑/更新/事件/活動）和搜尋欄
- 每個時代以章節呈現，內含年份分隔線

**標籤顏色（仍保留顯示）：**
- 里程碑：藍紫 `var(--sky)` + 放大圓點 + 卡片式框線
- 更新：綠色 `#64dcb4`
- 事件：橘色 `#ffaa32`
- 活動：紅色 `#ff8282`

### 首頁公告欄設計（v2 — 2026.04.14 重構）

**設計理念：** 公告應該像報紙頭條一樣直接可讀。

**架構：**
- 第一則公告為「精選卡片」（`.bulletin-featured`）：大張、直接展開、含標籤/日期/標題/內容/圖片
- 其餘公告為「緊湊列表」（`.bulletin-compact`）：全部展開、無需點擊，每則含圓點/日期/標題/標籤 + 展開的內容
- 底部保留「共 X 則公告」統計 +「查看全部」連結

**CSS 變數：**
- `.bulletin-featured` — 精選大卡
- `.bulletin-list` — 列表容器
- `.bulletin-compact` — 緊湊卡片
- 舊版 `.bulletin-item` / `.bulletin-toggle` / `.bulletin-body` 仍存在於 CSS 中（公告頁仍在用）
...
| 2026-04-13 | 公告頁重構 v4：展開式卡片 → 雜誌式閱讀器（文章直接展開、左側大日期） | 海風網站助手 |
| 2026-04-13 | 歷史館重構 v4：章回故事書風格（時代章節 + 敘事 + 左側圓點時間線） | 海風網站助手 |
| 2026-04-14 | 歷史館重構 v5：打掉重練，移除所有摺疊/展開機制，所有事件直接顯示；移除標籤篩選與搜尋欄，僅保留時代切換（全部/純麥/海風） | 海風網站助手 |
| 2026-04-14 | 首頁公告欄重構 v2：從摺疊式改為雜誌式卡片佈局，第一則為精選大卡（直接展開），其餘為緊湊列表（全部展開、無需點擊） | 海風網站助手 |
| 2026-04-14 | 全站 CSS/JS 版本號統一更新至 260414a | 海風網站助手 |
