# 海風 SeaWind 網站設計文件
| 2026.05.15 | 海風網站助手 | **手機導覽列按鈕無法點擊修復**：①遮罩層 `.nav-backdrop` 改為 `display: block` + `visibility: hidden` + `pointer-events: none`（原本 `display: none` 在手機瀏覽器上 transition 無效，且 `opacity: 0` 仍會攔截觸控事件）；②`.nav-backdrop.visible` 改用 `visibility: visible` + `pointer-events: auto` 控制顯示；③桌面版 `.dropdown-menu` 移除 `pointer-events: none/auto`（改由 `visibility: hidden/visible` 控制，避免手機版 CSS 特異性衝突）；④手機版 `.nav-dropdown .dropdown-menu` 關閉狀態補上 `pointer-events: none` 確保不攔截點擊；⑤手機版 `.nav-backdrop.visible` 移除多餘 `display: block`。cache busting → 150515f。頁尾日期 → 2026.05.15 |
