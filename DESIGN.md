# 海風 SeaWind 網站設計文件
| 2026.05.11 | 海風網站助手 | **浮動工具列音樂播放器重新設計**：①互動邏輯改為 hover 顯示面板＋點擊固定（pinned）＋再點取消固定；②面板加大（260px）並分為三區：曲目資訊區（圖示＋曲名＋藝人／專輯）、播放控制區（上一首／播放／下一首＋音量滑桿）、狀態列（播放狀態＋固定提示）；③背景改為更扎實的 #111827（移除所有透明度）；④hover 橋接區消除按鈕與面板間的 gap 閃爍；⑤播放中按鈕有呼吸光圈＋foam 綠色強調；⑥固定狀態有小圓點指示；⑦面板圖示隨播放狀態切換（音符→波形）。全站 87 頁 cache busting → 110511i。頁尾日期 → 2026.05.11 |
