...
| 2026.04.23 | 海風網站助手 | **首頁頁尾新增可愛小夥伴插圖**：在 `.footer-brand` 上方新增 `.footer-mascot` 圖片區塊（140×36px），使用 `<picture>` WebP fallback（9KB vs 60KB PNG）。hover 時微放大+上浮+opacity 提升。圖片置於 `assets/img/footer-mascot.{png,webp}` |
...
| 2026.04.25 | 海風網站助手 | **活動頁全面調整**：①活動列表頁移除所有舉辦人名字（👤 行）；②常駐活動卡片移除「常態開放」日期顯示；③珍奶日卡片移除日期（右上角 badge 已有）；④「暖身期」全站統一改為「調整期」（首頁橫幅+時間軸+活動頁 stages+倒數計時 JS）；⑤常駐活動 detail 頁（風汐系列、那個魚）新增基本資訊區塊（info-grid）；⑥生存起源 detail 頁移除舉辦者欄位；⑦攝影大賽 detail 頁移除評審欄位 |
