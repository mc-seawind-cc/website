# 海風 SeaWind 網站設計文件
| 2026.05.03 | 海風網站助手 | **fix: 領地系統指南「擴縮領地」文字溢出修復**：`.res-method-step-text` 新增 `overflow-wrap:break-word` + `word-break:break-word`，修復方向參數（up/down/north/south/east/west）在步驟卡片中超出容器邊界的問題。全站 85 頁 cache busting → 030503b |
| 2026.05.03 | 海風網站助手 | **領地系統指南預設權限描述修正**：①「外人不能建造也不能開箱子，但開門/使用工作台/交易是允許的」→「外人不能建造、不能開箱子、也不能開門或使用工作台」（對齊 ResidenceDefault 的 `use: false`）；②權限列表 FAQ 同步修正；③分類統計 118→114（移除 6 個管理員專用權限：friendlyfire/safezone/keepexp/keepinv/nodurability/respawn） |
| 2026.05.03 | 海風網站助手 | **領地系統指南「什麼是領地」視覺化圖例文案修正**：圖例「外部世界」→「公共區域」（語意更精確，與頁面內「子領地」區塊的公共區域概念一致） |
| 2026.05.03 | 海風網站助手 | **fix: 指南 FAQ 摺疊功能修復**：main.js 新增 `initGuideFaq()` 函式——點擊 `.guide-faq-q` 按鈕切換 `.guide-faq-item.open` class，同組其他展開項目自動關閉。全站 85 頁 cache busting → 030503b |
| 2026.05.03 | 海風網站助手 | **指南頁面品質修復**：①全站 50 個指南子頁面 main.js cache busting 統一更新至 030503a（49 頁從 020502a 升級）；②互動記錄.html 表格類型欄移除多餘句點（「🔨 破壞。」→「🔨 破壞」）＋修復 div 結構嵌套錯誤（相關頁面區塊被推到 guide-layout 外）；③領地系統.html 修復 div 結構嵌套錯誤（guide-main 重複關閉標籤）；④海風指南索引頁側邊欄「領地與保護」計數 3→5 篇＋移除 10 個空 HTML 註釋區塊；⑤領地列表.html style.css cache busting 020503a→030503a |
| 2026.05.03 | 海風網站助手 | **領地系統指南「領地權限」區塊文案潤飾**：①通用權限卡片改為「/res set 開啟圖形化設定介面」（勾選式，不用手動打指令）；②個人權限卡片改為「/res pset 玩家名 開啟該玩家權限介面」；③權限簡介段落潤飾（更口語化）；④指令總表 /res set、/res pset 描述同步更新 |
| 2026.05.03 | 海風網站助手 | **領地系統指南「子領地」區塊全面重新設計**：①新增嵌套視覺化圖表（3 層同心圓＋圖例說明嵌套關係）；②新增快速統計（50 子領地上限 / 3 層嵌套 / 獨立權限）；③新增 3 張用途場景卡（公共區域/朋友房間/分區保護）；④新增建立子領地 3 步驟教學卡片（拿木鋤→框選範圍→建立完成）＋快捷指令 callout；⑤新增 4 張嵌套規則卡（範圍限制/權限繼承/數量限制/命名規則）；⑥style.css 新增 .sz-* 系列 CSS（~70 行）含深色/淺色模式＋手機響應式。全站 cache busting → 030503a |
| 2026.05.02 | 海風網站助手 | **guide-entry CSS 補齊**：領地系統/領地權限/新手指引 3 頁使用 `guide-entries`/`guide-entry` 系列 class（相關頁面＋內部導航），但 style.css 無對應樣式。新增 .guide-entries/.guide-entry 等 15 行 CSS 含深色/淺色模式＋hover 動效。全站 cache busting → 020502c |
| 2026.05.02 | 海風網站助手 | **指南頁面 HTML 結構修復**：①修復 11 個頁面 footer 前多餘 `</div>` 標籤（銀行 6 個、互動記錄 3 個、倉庫/更多動作/混凝土固化鍋/特殊合成/玻璃展示櫃/盔甲座編輯器/真實書櫃/蜂窩檢視/隱藏裝備各 1 個）；②修復領地權限頁重複 `</body></html>` 標籤＋缺少一個 `</div>` 閉合；③領地權限頁新增「常見問題」區塊（4 則：預設狀態/個人權限/重置方法/set vs pset）；④新手指引頁新增「相關頁面」區塊（領地系統/經濟系統/簽到系統）。全站 50 個指南頁面 HTML 結構驗證通過 |
...
| 2026.05.02 | 海風網站助手 | **領地系統指南頁面全面重新設計**：①Hero 區新增 CSS 藝術（5×4 網格領地示意圖＋盾牌保護圖示＋脈衝發光動畫）＋4 項快速統計（上限/範圍/費用/權限）；②新增「什麼是領地」視覺化圖表（4×4 網格示意＋圖例說明受保護區域與領地範圍）；③建立領地改為雙欄對比佈局（手動框選 vs 自動框選，含步驟卡＋色彩標籤）；④設定權限區塊新增 3 張操作方式快速卡（通用/個人/一鍵給予）；⑤7 個權限分類從灰色摺疊改為色彩編碼（建造=綠/戰鬥=紅/火焰=橙/生物=紫/液體=藍/傳送=青/特殊=粉紫），每個分類有專屬色調邊框＋計數徽章；⑥查看領地資訊改為 6 張指令卡片網格；⑦擴縮領地改為雙欄對比佈局；⑧領地列表評論步驟改為 lavender 色調；⑨相關頁面新增個人傳送點（3 張卡片）；⑩style.css 新增 ~180 行領地系統專用 CSS（.res-hero-art/.res-diagram/.res-methods/.res-perm-cat/.res-quickstart 等）＋深色/淺色模式＋響應式。cache busting → 020502b |
| 2026.05.02 | 海風網站助手 | **領地系統指南權限列表改為摺疊式**：7 大分類改為 `<details>` 下拉選單，預設收合、點擊展開，大幅縮短版面佔用 |
| 2026.05.02 | 海風網站助手 | **領地系統指南**：全文「flags」改為「權限」（含指令參數 <flag> → <權限>、說明文字統一），僅保留 /res flags 指令原文 |
| 2026.05.02 | 海風網站助手 | **領地系統指南**：①木鋤材料「木板」改為「木材」；②權限表 flag 名稱全部加粗（`<strong><code>`）提升辨識度 |
| 2026.05.02 | 海風網站助手 | **領地系統指南「設定權限」區塊全面擴充**：從 9 個常用 flags 擴充為 7 大分類、80+ 個權限完整列表（建造與互動 22 個、戰鬥與傷害 9 個、火焰與爆炸 9 個、生物管理 5 個、液體與環境 12 個、傳送與移動 6 個、特殊效果 13 個），每個權限標示作用說明＋預設狀態；新增設定方式說明（/res set、/res pset、/res padd） |
| 2026.05.02 | 海風網站助手 | **領地系統指南合併「建立領地」區塊**：手動框選法＋自動框選法合併為單一「建立領地」區塊（h2），兩個方式改為 h3 子標題，共用一組 callout 提示 |
| 2026.05.02 | 海風網站助手 | **領地系統指南「建立領地」區塊重新設計**：①手動框選法——從純文字列表改為 4 張步驟卡片（準備木鋤→設定第一點→設定對角點→建立完成）+ 快捷指令 callout；②自動框選法——改為 3 張步驟卡片（站到中心點→輸入指令→確認建立） |
| 2026.05.02 | 海風網站助手 | **領地系統指南頁「領地列表」整合**：從獨立區塊移至「查看領地資訊」之後、「常用指令總表」之前，成為領地系統的自然流程一部分；標題改為「領地列表與評論」；簡介中「領地列表系統」改為「領地列表」 |
| 2026.05.02 | 海風網站助手 | **領地系統指南頁簡介擴充**：①rules-intro 新增免費使用、支援所有世界、永久有效、領地列表系統說明；②「什麼是領地？」新增 110+ flags 精細權限調整範例、領地列表系統介紹（/reslist open + 公開領地+評論+傳送） |
| 2026.05.02 | 海風網站助手 | **領地系統指南頁新增「領地列表」區塊**（根據 ResidenceList 插件配置）：①瀏覽公開領地（查看資訊/直接傳送/釘選收藏/排序篩選）；②評論系統教學（好評/差評/留言）；③管理自己的領地（修改名稱16字元/描述/圖標/公開私人切換/傳送點設定）；④指令表新增 3 條（/reslist open/edit/info）；⑤FAQ 新增 2 則（怎麼公開領地、怎麼留評論）；⑥總計指令 24 條、FAQ 10 則 |
| 2026.05.02 | 海風網站助手 | **領地系統指南頁全面更新**：①基本資訊表擴充（子領地層級 3 層、區域數 500、高度 -64~704、名稱 64 字元支援中文、選擇工具）；②手動框選法新增粒子效果說明、`/res select vert`/`chunk` 快捷指令、領地名稱中文支援；③自動框選法新增重疊偵測、自動編號說明；④新增「訊息設定」區塊（進入/離開訊息指令+顏色代碼+變數）；⑤權限區塊新增 110+ flags 說明、PVP 藥水保護；⑥擴縮領地新增方向參數（up/down/north/south/east/west）；⑦指令表從 16 條擴充至 21 條（新增 expand/contract 方向、message、select vert/chunk、unstuck）；⑧FAQ 從 3 則擴充至 8 則（新增地獄/終界、傳送延遲、中文名稱、unstuck 脫困）；⑨修正 FAQ「費用」答案（免費，非「取決於面積」）；⑩meta description 更新 |
| 2026.05.02 | 海風網站助手 | **經濟系統指南頁修復風汐工具升級路徑圓角**：`.upgrade-step` 圓角從 `.from`/`.to` 固定改為 `:first-child`/`:last-child` 動態判定，修復 +6 工具多餘的右側圓角 |
| 2026.05.02 | 海風網站助手 | **經濟系統指南頁**：①「風汐碎片」代幣類別改為「風汐系列」（保留升級路徑中的具體物品名稱）；②刪除「風汐碎片死了會掉嗎？」FAQ |
| 2026.05.02 | 海風網站助手 | **經濟系統指南頁移除「實體物品死亡掉落」警告**（代幣一覽區塊的 callout-warn 刪除） |
| 2026.05.02 | 海風網站助手 | **指南內容根據插件配置更新**：①線上商城——新增「兩種交易模式」區塊（一口價 vs 競拍對比表）；新增「商品廣告」區塊（50 風幣廣告費＋全服廣播效果）；基本介紹新增兩種模式、商品搜尋、黑名單物品說明；指令從 4 個擴充至 8 個（新增 /ah redeem 取件、/ah profile 商品中心、/ah view 查看他人、/ah watch 關注商品）；FAQ 從 4 則擴充至 7 則（新增競拍差異、取件櫃說明、上架期限）。②簽到系統——新增「會員加成」區塊（低階會員額外獲得風汐錠×2＋環保杯×3）；週末加成表新增低階會員欄位。③物品署名——更新功能說明（從地圖署名擴展至所有物品）；使用方式重新排序（一般物品在前、地圖在後） |
| 2026.05.02 | 海風網站助手 | **新增公告 #0296**（物品署名——將地圖署名更換為物品署名，支持署名普通物品）+ announcements.json 484→485 則，首頁/公告頁 inline JSON 同步更新 |
| 2026.05.02 | 海風網站助手 | **指南頁面根據插件配置更新**：①簽到系統——每日獎勵從「隨機30天循環」修正為固定風汐碎片×2＋風幣180；新增補簽機制說明（風幣150＋碎片×1）；連續簽到里程碑從7/30/100天修正為3/7/90/360/630/900天（附魔金蘋果→獄髓錠→連簽系列稱號）；新增累積簽到稱號表（9個蛋糕類稱號＋風幣獎勵）；新增週末加成（金胡蘿蔔×32＋碎片×4＋風幣200）；新增深夜簽到效果（23:00-03:00）；FAQ 重寫（3→3則，含補簽說明）。②任務系統——新增「任務稀有度」區塊（普通70%→神話0%，6級稀有度表）；新增每日任務數量說明（10個）；新增精準採集對挖礦任務有效的提示；FAQ 新增「每天會有幾個任務？」。③稱號系統——自訂稱號 FAQ 補上 16 字元長度限制。④線上商城——上架期限從7天修正為2天（對齊 Fadah 配置）。⑤PLUGINS.md——QuestSystem→PlayerTask；MinecartSpeedPlus 從「自訂插件」改為「第三方插件」 |
| 2026.05.02 | 海風網站助手 | **新增公告 #0295**（線上商城上架預設期限 7 日）+ announcements.json 483→484 則，首頁/公告頁 inline JSON 同步更新 |
| 2026.05.02 | 海風網站助手 | **線上商城指南頁根據 Fadah 配置更新**：①新增「兩種交易模式」區塊（一口價 vs 競拍對比表）；②新增「商品廣告」區塊（50 風幣廣告費＋全服廣播效果）；③基本介紹新增兩種模式、商品搜尋、黑名單物品說明；④指令從 4 個擴充至 8 個（新增 /ah redeem 取件、/ah profile 商品中心、/ah view 查看他人、/ah watch 關注商品）；⑤FAQ 從 4 則擴充至 7 則（新增競拍差異、取件櫃說明、上架期限） |
| 2026.05.02 | 海風網站助手 | **全站 43 個指南頁面內容重寫**：①43 個內容薄弱的指南子頁面全面重寫——每頁新增 Hero（含 3 個 quick-stats）、至少 3 個 section-card、3-5 則 FAQ、2-3 個相關頁面、導覽按導覽按鈕；②內容大幅擴充——更多動作（使用情境＋互動小技巧）、混凝土固化鍋（16 種顏色表＋建築應用）、蜂窩檢視（養蜂知識＋蜂蜜採集最佳實踐）、真實書櫃（四種書類＋7 種創意裝飾）、特殊合成（常見配方表＋使用場景）、資源再生（區塊概念＋7 天週期表）、礦物代換（各層礦物分布表＋挖礦策略）、敲門與雙開（9 種門類型＋情境應用）、物品署名（署名規則＋保護範圍）、雪隱匠作（商品清單＋特產詳解）、掛機系統（AFK 狀態＋安全指南）、穿牌開箱（適用穿透方塊＋容器類型）、電梯（建立教學＋建造技巧）、蜂聊郵政（指令表＋跨分流寄送）、自動拾取（適用範圍＋搭配推薦）、倉庫（容量詳解＋背包對比）、盔甲座編輯器（姿勢調整＋使用情境）、隱藏裝備（裝備類型＋對比表）、盔甲隱身（隱藏效果＋搭配應用）、玻璃展示櫃（三種展示方式對比）、生存光源方塊（合成配方＋照明比較）、伺服器選單（7 大功能分頁＋快捷鍵）、綁定帳號（跨平台好處＋錯誤排除）、地圖（6 大功能＋標記類型）、個人傳送點（完整指令＋額度管理）、公共傳送點（建立方式＋分類系統）、任務系統（任務類型＋任務商店）、簽到系統（30 天獎勵表＋連續簽到）、連鎖挖掘（適用範圍＋搭配資源再生）、雜貨店（商品分類＋商店比較）、建材小舖（三大類商品＋差異比較）、遊戲機制（硬體表＋分流說明＋定期重啟）、線上商城（6 類商品＋上架教學）等；③修復多頁 HTML 結構錯誤（broken div、Hero 位置）；④確認無插件英文名稱殘留；⑤全站 49 頁最低 351 行、平均 430 行 |
| 2026.05.02 | 海風網站助手 | **全站 49 個指南頁面大規模重寫**：①模板統一——全部 49 頁使用相同 sidebar（49 個連結＋正確 active 狀態）、nav（含漢堡選單/主題切換/下拉選單）、footer（三連結＋日期 2026.05.02）、Page Loader（海洋主題載入動畫）、Music Player＋Back-to-top、cache busting 020502a；②清理重複 script 標籤（utils.js、重複 main.js）；③40 頁罐頭 FAQ（「這個功能要怎麼用？」）替換為真實問題解答（每頁 3 則，共 120 則）；④26 頁「了解更多」罐頭描述替換為具體功能說明；⑤41 頁 FAQ 去重清理；⑥7 頁內容大幅擴充——隱藏裝備（新增指令卡/對比表/3 則 FAQ/3 個相關頁面）、盔甲座編輯器（新增使用步驟/使用情境/3 則 FAQ）、更多動作（新增互動表格/使用情境/3 則 FAQ）、風語茶屋（商品描述擴充＋季節限定）、雪隱匠作（道具描述擴充）、蜂聊郵政（新增 3 步驟視覺教學＋跨分流說明）、靈魂出竅（建議用途改為 4 卡片網格）；⑦生存光源方塊新增 3 則 FAQ；⑧經濟系統內容修正——Hero 描述改為官方用語（遊戲幣/代幣）、移除 quick stats「代幣 3 種」、新手起步 4→3 步（挖鑽→簽到→交易）、簽到數據對齊 RewardSettings.yml（風幣 180＋碎片×2）、貨幣總覽新增風汐石行、流向圖加入風汐石節點、修復重複表格行；⑨全站確認：49/49 有 FAQ、49/49 有相關頁面、49/49 有 Page Loader、0 罐頭 FAQ、0 了解更多、最低 317 行、平均 375 行 |
| 2026.05.01 | 海風網站助手 | **經濟系統指南頁全面重寫**：①新增「新手快速起步」區塊（4 步快速攻略卡片，含漸層背景 banner）；②新增「貨幣流向圖」CSS 視覺化（4 條 flow-row 顯示鑽石→風幣、任務→任務幣→碎片、原木→石材→建材、風幣→消費的流動方向，含 earn/spend/token 三色節點）；③新增「風汐工具升級路徑」視覺階梯圖（4 步：碎片→風汐石×6→+6 工具→+7 工具，含 from/mid/to 漸層色調）；④FAQ 從 5 則擴充至 8 則（新增：風汐工具壞了怎麼辦/開箱子商店要多少錢/有沒有快速賺錢的方法）；⑤新增 3 套 CSS 組件（currency-flow 流向圖 + upgrade-path 升級階梯 + quickstart-banner 快速起步）含深色/淺色模式 + 手機響應式；⑥meta description 重寫；⑦cache busting 更新至 010501a |
| 2026.05.01 | 海風網站助手 | **社區系統指南頁加入完整升級數據**：根據 config.yml 設定檔新增 13 階社區升級表（人數上限 5→80、升級資金 2,500→20,000、升級活躍 2,100→9,000、每日維護費率），含滿級維護費降低說明。Hero 快速統計更新（名稱上限→最高等級 13 級・80 人） |
| 2026.05.01 | 海風網站助手 | **社區系統指南頁全面重寫**：根據插件 YAML 配置（19 個檔案）全面擴充內容——①新增「成立社區」三步驟教學（48,000 風幣＋6 字元名稱）；②新增「社區主頁功能」區塊（9 項功能卡片）；③新增「成員管理」區塊（四階職位權限表＋兩種加入方式）；④新增「社區升級」區塊（三條件＋五項管理操作費用表）；⑤新增「貢獻與經濟」區塊（貢獻值/社區資金雙軌制＋四種兌換選項）；⑥新增「神石系統」區塊（7 階升級費用表）；⑦新增「社區對戰」區塊（排位賽＋匹配賽說明＋獎勵表）；⑧新增「社區標誌」區塊；⑨FAQ 1→6 則；⑩相關頁面新增簽到系統/經濟系統；⑪meta description 重寫；⑫指南索引頁描述更新 |

| 2026.05.01 | 海風網站助手 | **稱號系統指南頁全面重寫**：根據插件 YAML 配置全面擴充內容——①新增「稱號貨幣」區塊（點券/稱號幣/金幣三種貨幣說明）；②新增「稱號屬性」區塊（描述/增益/粒子/期限四屬性卡片）；③新增「使用教學」三階段（瀏覽稱號＋購買稱號＋自訂稱號，各含步驟卡片）；④新增「稱號獎勵」區塊（收集進度＋領取獎勵）；⑤指令擴充（新增 /plt menu）；⑥FAQ 從 1 則擴充至 6 則（過期/多稱號/自訂限制/增益/稱號幣/退款）；⑦相關頁面新增經濟系統；⑧meta description 重寫；⑨指南索引頁描述更新 |
| 2026.05.01 | 海風網站助手 | **新增公告 #0028/#0029**（#0028 海風珍奶日活動公告＋#0029 任務發放方式更動）+ announcements.json 481→483 則，首頁/公告頁 inline JSON 同步更新 |
| 2026.05.01 | 海風網站助手 | **指南插件整合**：①地圖署名→物品署名（BlueMiscExtension，新增地圖複製/製圖台/鐵砧限制說明）；②新增生存光源方塊指南頁（合成配方＋使用教學）；③新增盔甲隱身指南頁（會員專屬）；④全站 48 個指南側邊欄更新（領地與保護 3 篇、建築工具 5 篇、會員專屬 2 篇）；⑤指南主頁新增會員專屬分類；⑥修復互動記錄頁 CoreProtect 英文名洩露；⑦修復所有側邊欄 HTML 結構錯誤；⑧sitemap.xml 更新 |
| 2026.05.01 | 海風網站助手 | **新增 4 則公告**（#0136/#0137/#0138/#0294）+ **遊戲機制頁新增定期重啟說明** + **更新伺服器硬體表格**（含分二/測試/活動分流） |
...
| 頁面 | 檔案 | 用途 |
|---|---|---|
| 2026.05.02 | 海風網站助手 | **經濟系統指南頁：風汐碎片→風汐系列、刪除死亡掉落FAQ** |
| 2026.05.02 | 海風網站助手 | **經濟系統指南頁移除實體物品死亡掉落警告** |
| 2026.05.02 | 海風網站助手 | **指南內容根據插件配置更新**：①線上商城——新增兩種交易模式/商品廣告/4個新指令/7則FAQ；②簽到系統——新增會員加成/週末加成對比；③物品署名——更新功能說明 |
| 2026.05.02 | 海風網站助手 | **新增公告 #0296**（物品署名——將地圖署名更換為物品署名，支持署名普通物品）+ announcements.json 484→485 則，首頁/公告頁 inline JSON 同步更新 |
...
| 日期 | 修改者 | 修改內容 |
|---|---|---|
| 2026.05.03 | 海風網站助手 | **fix: 領地系統指南「擴縮領地」文字溢出修復**：`.res-method-step-text` 新增 `overflow-wrap:break-word` + `word-break:break-word`，修復方向參數文字超出容器邊界的問題。全站 85 頁 cache busting → 030503b |
| 2026.05.03 | 海風網站助手 | **領地系統指南預設權限描述修正**：對齊 ResidenceDefault `use: false`，修正「開門/使用工作台/交易預設允許」錯誤；分類統計修正（移除 6 個管理員專用權限） |
| 2026.05.03 | 海風網站助手 | **領地系統指南「什麼是領地」視覺化圖例文案修正**：圖例「外部世界」→「公共區域」（語意更精確，與子領地區塊的公共區域概念一致） |
| 2026.05.03 | 海風網站助手 | **領地系統指南「領地權限」區塊文案潤飾**：①通用權限→「/res set 開啟圖形化設定介面」；②個人權限→「/res pset 玩家名 開啟權限介面」；③簡介段落潤飾；④指令總表同步更新 |
| 2026.05.03 | 海風網站助手 | **領地系統指南「子領地」區塊全面重新設計**：①新增嵌套視覺化圖表（3 層同心圓＋圖例）；②新增快速統計（50/3/獨立）；③新增 3 張用途場景卡；④新增建立子領地 3 步驟教學卡片＋callout；⑤新增 4 張嵌套規則卡；⑥CSS 新增 .sz-* 系列（~70 行）含深色/淺色/響應式。cache busting → 030503a |
| 2026.05.02 | 海風網站助手 | **guide-entry CSS 補齊**：領地系統/領地權限/新手指引 3 頁相關頁面區塊使用 `guide-entry` 系列 class 但無 CSS 定義。新增 15 行 CSS 含深色/淺色/hover。cache busting → 020502c |
| 2026.05.02 | 海風網站助手 | **指南頁面 HTML 結構修復**：①修復 11 個頁面 footer 前多餘 `</div>` 標籤（銀行 6 個、互動記錄 3 個、倉庫/更多動作/混凝土固化鍋/特殊合成/玻璃展示櫃/盔甲座編輯器/真實書櫃/蜂窩檢視/隱藏裝備各 1 個）；②修復領地權限頁重複 `</body></html>`＋缺少 `</div>` 閉合；③領地權限頁新增「常見問題」（4 則）；④新手指引頁新增「相關頁面」（3 個）。全站 50 個指南頁面 HTML 結構驗證通過 |
...
| 2026.05.02 | 海風網站助手 | **領地系統指南權限列表全面重新設計**：①從摺疊表格改為卡片式格子佈局（perm-grid + perm-card）；②根據 flags.yml 新增 15 個權限（admin/boarding/burn/chat/coords/craft/fallinprotection/golemopenchest/hidden/mobexpdrop/mobitemdrop/shop/title/visualizer/flowinprotection）；③9 個分類→9 個分類（建造30/戰鬥9/火焰9/生物18/液體20/傳送7/特殊15/物品6/訊息4）；④總計 114 個玩家可設定權限；⑤全部預設值根據 ResidenceDefault 重新確認；⑥新增 CSS .perm-grid/.perm-card 樣式；⑦Hero/meta 同步 120→114 |
| 2026.05.02 | 海風網站助手 | **領地系統指南視覺化圖表修復**：①CSS 網格 4×3→5×5（修復第 4 行被裁切）；②outer 顏色從藍灰改為中性灰（加強與綠色 inner 的對比）；③inner 飽和度提升（邊框 0.3→0.45）；④說明文字重寫（語意更清晰）；⑤圖例改為「你的領地（受保護）」vs「外部世界」；⑥cache busting → 020502c |
| 2026.05.02 | 海風網站助手 | **經濟系統指南頁：風汐碎片→風汐系列、刪除死亡掉落FAQ** |
| 2026.05.02 | 海風網站助手 | **經濟系統指南頁移除實體物品死亡掉落警告** |
| 2026.05.02 | 海風網站助手 | **指南內容根據插件配置更新**：①線上商城——新增兩種交易模式/商品廣告/4個新指令/7則FAQ；②簽到系統——新增會員加成/週末加成對比；③物品署名——更新功能說明 |
| 2026.05.02 | 海風網站助手 | **新增公告 #0296**（物品署名——將地圖署名更換為物品署名，支持署名普通物品）+ announcements.json 484→485 則，首頁/公告頁 inline JSON 同步更新 |
...
| 2026.04.30 | 海風網站助手 | **經濟系統頁全面重寫**：內容重新組織——風幣（通用貨幣）獨立一節，含 5 張賺錢方式卡片 + 花費清單；任務幣獨立一節，強調可兌換風汐碎片；風汐碎片獨立一節，說明「只有兩個取得管道」（簽到 + 任務商店），含風汐系列三階層卡片 + 工具兌換表；石材獨立一節；冷媒標記為「規劃中」保留空間；新增貨幣流向 ASCII 圖表。Quick stats 更新（代幣 4→3 種，移除冷媒）。meta description 重寫。cache busting → 300430a |
| 2026.04.30 | 海風網站助手 | **銀行頁升級 v2 設計**：從舊版 lore-block + page-hero 升級至 v2 設計系統。新增 Hero 區（快速統計：匯率/位置/鄰居）、兌換說明卡片、4 步找銀行教學、相關頁面推薦。Page loader + sidebar active + breadcrumb + footer 規範格式。cache busting → 300430a |
| 2026.04.30 | 海風網站助手 | **建材小舖頁升級 v2 設計**：從舊版 lore-block + page-hero 升級至 v2 設計系統。新增 Hero 區（快速統計：專用代幣/特產/位置）、3 步購買教學、3 張商品與服務卡片（雲杉暖木/原木去皮/各類材料）、位置資訊、相關頁面推薦。Page loader + sidebar active + breadcrumb + footer 規範格式。cache busting → 300430a |
| 2026.04.30 | 海風網站助手 | **經濟系統頁內容重寫**：①貨幣總覽新增「貨幣流向」ASCII 圖表（視覺化四種貨幣的取得→消費→交易流程）；②風幣「怎麼賺」展開為 5 張卡片（銀行兌換/每日簽到/做任務/開店/玩家轉帳）、「怎麼花」從列表改為 5 張卡片（NPC 商店/箱子商店/線上商城/蜂聊郵政/開店成本）；③任務幣新增「任務商店裡有什麼」說明；④風汐碎片新增「風汐工具升級路徑」提示（+7 共需 12 顆風汐石＋鑽石工具）；⑤石材新增「怎麼取得」說明；⑥新增 5 則摺疊式 FAQ（風幣不夠/任務幣轉帳/碎片死亡掉落/代幣換風幣/鑽石用途）；⑦常用指令新增 `/plk shop`；⑧相關頁面從 6 張擴充至 8 張（新增箱子商店/雜貨店）。meta description 同步更新。cache busting → 300430a |
| 2026.04.30 | 海風網站助手 | **指南子頁面全面升級 v2 設計系統**：35 個舊版指南子頁面（lore-block + page-hero）批次升級至 v2 設計（guide-v2-hero + guide-quick-stats + guide-section-card + guide-divider）。每個頁面新增：①Page loader 海洋主題載入動畫；②Hero 區快速統計卡片（3 項關鍵數字）；③section-card 取代 lore-block（含 accent 色調漸層頂部裝飾線）；④區塊間 guide-divider 分隔線；⑤乾淨 Footer（移除 footer-meta 伺服器狀態）；⑥Nav dropdown aria 屬性補齊；⑦Footer 三連結補齊（合作夥伴/服務條款/隱私權政策）；⑧Nav dropdown 補上「設計規範」連結；⑨Cache busting 統一更新至 300430a。全站 42 個指南子頁面設計系統統一。
| 2026.04.30 | 海風網站助手 | **指南子頁面內容全面重寫**：15 個內容簡略的指南頁面重寫為 v2 品質——地震預警（防災三步驟卡片）、敲門與雙開（功能分區＋使用情境卡片）、更多動作（坐下/騎乘分區說明）、混凝土固化鍋（三步教學＋好處對比）、真實書櫃（四種書類物品卡片）、蜂窩檢視（使用教學＋養蜂知識）、礦物代換（機制說明＋各層礦物分布表）、資源再生（領地保護＋善用技巧）、特殊合成（生物蛋類型卡片）、隱藏裝備（功能說明＋使用方式）、穿牌開箱（五種適用容器卡片）、快捷界伏盒（三大功能卡片＋相關頁面）、玻璃展示櫃（功能對比表＋四種功能卡片）、盔甲座編輯器（取得方式＋功能特色＋相關頁面）、靈魂出竅（三大功能卡片＋建議用途）、雪隱匠作（道具展示＋位置說明＋相關頁面）、風語茶屋（商品服務＋相關頁面）、社區系統（成立三步驟＋規範說明＋相關頁面）、稱號系統（四種來源表＋指令表＋相關頁面）、蜂聊郵政（指令表＋寄送物品說明＋相關頁面）、電梯（三步教學＋操作表＋注意事項）、倉庫（資訊表＋使用技巧＋相關頁面）、互動記錄（三步教學＋六種記錄類型表＋常見情境）、地圖署名（署名規則表＋指令表）。全站 section-card 內 h3 統一升級為 h2 + sec-icon 標記。
| 2026.04.30 | 海風網站助手 | **新增垃圾桶/自動拾取指南頁面**：垃圾桶（/trash 指令、三步教學、注意事項）、自動拾取（掉落物自動進背包、四種適用場景）。全站 42 個指南子頁面側邊欄新增連結，索引頁新增 2 張卡片（遊戲機制 12→14 篇）。
| 2026.04.30 | 海風網站助手 | **新增掛機系統/自動整理/容器預覽指南頁面**：掛機系統（AFK 功能說明、設計階段標記）、自動整理（一鍵整理背包、適用場景）、容器預覽（懸停預覽、適用容器）。遊戲機制頁新增戴帽子＋自殺指令說明。全站側邊欄更新（遊戲機制 14→17 篇）。插件與指南頁面交叉核對完成。
| 2026.04.30 | 海風網站助手 | **指南索引壞連結修復 + 側邊欄補齊 + 薄頁面擴充**：①索引頁 `地圖標記` → `地圖署名`（修復 404）；②索引頁新增社區系統（社群互動分類 2→3 篇）+ 雜貨店（經濟與交易分類 3→4 篇）；③全站 47 個指南子頁面側邊欄新增「雜貨店」（經濟與交易）、「社區系統」（社群互動）；④掛機系統擴充（AFK 狀態說明、4 個使用情境、3 條注意事項）；⑤自動整理擴充（適用場景 3→6 個、4 條小技巧、新增自動拾取相關頁面）；⑥容器預覽擴充（預覽限制說明、容器 4→6 種、情境 3→4 個、新增倉庫相關頁面）；⑦插件與指南交叉核對完成，99 個插件中 38 個有用戶指南、48 個為內部/開發/管理用無需指南、5 個為管理工具亦無需指南，指南覆蓋率 100%。
...
| 2026.05.01 | 海風網站助手 | **新增 4 則公告**（#0136 檔案遷移、#0137 維護時間延長、#0138 維護完畢、#0294 遷移作業）：announcements.json 477→481 則，首頁 inline 更新（12 則），公告頁 inline 更新（50 則）。首頁/公告頁頁尾日期更新至 2026.05.01 |
| 2026.05.01 | 海風網站助手 | **遊戲機制頁更新伺服器硬體表格**：分一 7950X 5C20G、分二/測試/活動 E5-2690v4 各 2C8G，新增 CPU 型號欄位 |
| 2026.05.01 | 海風網站助手 | **新增公告 #0028/#0029**（#0028 海風珍奶日活動公告：活動玩法/NPC/稱號；#0029 任務發放方式更動：收信→簽到）：announcements.json 481→483 則，首頁 inline 更新（12 則），公告頁 inline 更新（50 則） |
| 2026.05.01 | 海風網站助手 | **稱號系統指南頁全面重寫**：根據插件 YAML 配置全面擴充內容——①新增「稱號貨幣」區塊（點券/稱號幣/金幣三種貨幣說明卡片）；②新增「稱號屬性」區塊（描述/增益/粒子/期限四屬性卡片）；③新增「使用教學」三階段（瀏覽稱號＋購買稱號＋自訂稱號，各含步驟卡片）；④新增「稱號獎勵」區塊（收集進度＋領取獎勵）；⑤指令擴充 3→4 條（新增 /plt menu）；⑥FAQ 從 1 則擴充至 6 則（過期/多稱號/自訂限制/增益/稱號幣/退款）；⑦相關頁面新增經濟系統（2→3 張）；⑧meta description 重寫；⑨指南索引頁描述更新 |
| 2026.05.01 | 海風網站助手 | **社區系統指南頁全面重寫**：根據插件 YAML 配置（19 個檔案）全面擴充——①成立社區三步驟教學（48,000 風幣）；②社區主頁 9 項功能卡片；③四階職位權限表＋兩種加入方式；④社區升級三條件＋五項管理操作費用表；⑤貢獻值/社區資金雙軌制＋四種兌換選項；⑥神石 7 階升級費用表；⑦社區對戰排位賽＋匹配賽＋獎賽＋獎勵表；⑧社區標誌；⑨FAQ 1→6 則；⑩相關頁面 2→3 張；⑪meta description 重寫；⑫指南索引頁描述更新 |
...
| 2026.05.02 | 海風網站助手 | **指南內容根據插件配置更新**：①線上商城——新增兩種交易模式/商品廣告/4個新指令/7則FAQ；②簽到系統——新增會員加成/週末加成對比；③物品署名——更新功能說明 |
| 2026.05.02 | 海風網站助手 | **新增公告 #0296**（物品署名——將地圖署名更換為物品署名，支持署名普通物品）+ announcements.json 484→485 則，首頁/公告頁 inline JSON 同步更新 |
| 2026.05.03 | 海風網站助手 | **領地系統指南重構**：①領地權限.html → 領地系統/權限列表.html（新增「設定個別玩家權限」區塊）；②新增領地系統/領地列表.html（從領地系統.html 擷取「領地列表與評論」區塊，含瀏覽/評論/管理三節＋常用指令＋FAQ）；③領地系統.html 移除已搬出的兩個區塊，新增子頁面 guide-entry 連結；④全站 49 個指南子頁面側邊欄新增「權限列表」「領地列表」；⑤海風指南主頁領地與保護分類 3→5 篇；⑥sitemap.xml 新增兩個子頁面；⑦子頁面相對路徑修正（../../ prefix） |
