# 海風 SeaWind 網站設計文件
| 2026.05.16 | 海風網站助手 | **線上工單頁新增工單計數器**：①Hero 區新增 `ticket-stats` 統計條——左側「已處理工單」數字（從 `ticket-count.json` fetch，easeOut 動畫）、右側「平均回覆時間 ~7 天」；②新增 `ticket-count.json`（初始 0，團隊手動更新）；③CSS 含 tabular-nums 等寬數字＋分隔線＋響應式。cache busting → 160516a。頁尾日期 → 2026.05.16 |
