...
| 日期 | 修改者 | 修改內容 |
|---|---|---|
| 2026.05.03 | 海風網站助手 | **領地系統指南「領地權限」區塊全面重寫**：①新增「權限三態」說明（允許/拒絕/移除）＋三張狀態卡；②三種設定方式從一句話擴充為完整說明（通用 /res set：勾選式 GUI；個人 /res pset：覆蓋通用設定；快速 /res padd：一鍵開放）；③新增「實際操作流程」雙欄佈局（padd 3 步 vs pset 3 步）；④新增 callout 混用技巧提醒 |
