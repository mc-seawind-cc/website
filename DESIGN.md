...
### 特殊頁面
- `404.html` — 精美 404 頁面（月夜海景 + 8 個小遊戲）
...
2. **首頁 Hero 圖片同時有 PNG 和 WebP** — `assets/img/homeHero.png` 和 `assets/img/homeHero.webp` 都存在，但 HTML preload 引用 PNG，CSS background 也用 PNG。建議改用 WebP + PNG fallback：
   ```html
