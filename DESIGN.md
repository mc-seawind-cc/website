...
- **4 級進度分層**（新芽→居民→資深→傳說），依遊玩程度遞進
- 每個任務有創意名稱＋目標類型＋數量
- 進度分層透過 LuckPerms 權限實現（`playerTask.rarity.*.*`）

### 稀有度一覽

| 稀有度 | config 名稱 | 顏色 | 任務幣 | 風幣 | 普通機率 | 傳說機率 |
|---|---|---|---|---|---|---|
| ⚪ 普通 | normal | #FFFFFF | 2 | 100 | 70% | 35% |
| 🔵 進階 | advanced | #FFFF55 | 6 | 140 | 20% | 55% |
| 🟣 稀有 | rare | #55FFFF | 10 | 200 | 8% | 8% |
| 🟠 史詩 | rarer | #FF55FF | 64 | 500 | 2% | 2% |
| 🟢 冒險 | mythical | #4ab652 | 2 | 依各任務 | 0% | 0% |

> 設定檔中另有 `epic`（#a8e6cf）目前未啟用。
> 傳說階段：進階 55%、普通 35%，老更容易遇到高獎勵任務。

### 進度分層

最後一個階段（傳說）解鎖全部稀有度，按照機率分配每日任務。

| 等級 | 條件 | 解鎖稀有度 | config 權限 |
|---|---|---|---|
| 🌱 新芽 | ~1 週 | 普通 | normal |
| 🌿 居民 | 1 週~1 月 | + 進階 | + advanced |
| 🌳 資深 | 1~3 月 | + 稀有 | + rare |
| 🏔️ 傳說 | 3 月+ | 全部（按機率分配） | + rarer + mythical |
