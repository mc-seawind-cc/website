# 海風 SeaWind 網站設計文件
| 2026.05.03 | 海風網站助手 | **領地系統指南「擴縮領地」步驟文案精簡**：移除步驟 1 中「，或在指令中指定（up/down/north/south/east/west）」及「，或在指令中指定方向」——指令區塊已有 `[方向]` 參數說明，步驟中無需重複。全站 85 頁 cache busting → 030503b |
...
| 日期 | 修改者 | 修改內容 |
|---|---|---|
| 2026.05.03 | 海風網站助手 | **fix: 領地系統指南「擴縮領地」文字溢出修復**：移除步驟中重複的方向參數說明（指令區塊已有 `[方向]`），`.res-method-step-text` 新增 `overflow-wrap:break-word`。全站 85 頁 cache busting → 030503b |
