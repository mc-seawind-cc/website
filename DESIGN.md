...
| 2026-04-18 | 海風網站助手 | 📊 首頁新增玩家數量歷史趨勢 sparkline：每次載入首頁時 savePlayerSnapshot() 儲存玩家數至 localStorage（保留最近 48 筆、10 分鐘去重），fetchServerStatus 成功後自動繪製 Canvas sparkline（area fill + line + 最後點 dot + max/min 標籤）。深色/淺色主題同步，切換主題自動重繪。canvas 元素嵌入伺服器狀態 info-card 內，至少 2 筆資料才顯示 |
