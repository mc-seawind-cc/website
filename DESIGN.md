# 海風 SeaWind 網站設計文件
| 2026.05.14 | 海風網站助手 | **贊助頁季卡定位重調整（VIP vs 季節通行證）**：①季卡卡片：從「9 項功能，到期自動關閉」改為主打「活動＋季節額外獎勵」，隨地開啟降為附贈功能；②會員卡片：「22 項永久回饋」→「VIP 永久身份」；③組合卡片：「一次解鎖全部 31 項回饋」→「一次解鎖 VIP 永久身份＋當季活動加成」；④對比表：季卡專屬區從底部移至待設計前（更顯眼），隨地開啟分類標籤從「會員永久/季卡到期關閉」→「兩個方案都有」；⑤FAQ 重寫——會員vs季卡差別改為「長期投資 vs 活動季加成」、季卡到期答案補充活動/季節獎勵也會關閉、組合方案改用 VIP 術語；⑥贈卡文案同步（季卡功能體驗＋不含 VIP 身份）；⑦meta/og description 同步更新。cache busting → 140514f。頁尾日期 → 2026.05.14 |
