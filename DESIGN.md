# 海風 SeaWind 網站設計文件
| 2026.05.14 | 海風網站助手 | **新增「季節」列表頁＋全站導覽列更新**：①新建 `季節.html`——對標活動頁的季節系統列表頁，含 Hero（季節統計條）/季節 vs 活動比較/季節卡片（風緣季 featured）/FAQ（4 則）；②全站 92 頁導覽列「關於」下拉選單新增「季節」連結（root/1-level/2-level 三層路徑）；③風緣季頁面返回連結改為「← 返回季節列表」；④sitemap.xml 新增季節.html + 季節/風緣季；⑤cache busting → 140514h。頁尾日期 → 2026.05.14 |
