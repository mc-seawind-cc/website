...
#### 2. CSS `-webkit-backdrop-filter` 不完整
- **嚴重程度**：中
- **影響**：15 處 `backdrop-filter` 缺少 `-webkit-` 前綴，Safari/iOS 上毛玻璃效果失效
- **位置**：導覽列、下拉選單、公告卡片等
- **建議**：統一補上前綴

#### 3. `execCommand('copy')` 已棄用
- **嚴重程度**：低
- **位置**：`main.js` 的 `copyServerIP()` 函式
- **建議**：改用 `navigator.clipboard.writeText()`，保留 fallback
...
| 舊維護（已停用） | `1330216386613743730` | 獨立 | 不再更新，僅供歷史參考 |
