...
季度系統是海風的季節性活動框架。每個季度持續約 10 週，所有玩家免費參加，季卡玩家享額外獎勵。
...
|---|---|
| 名稱 | **風緣季** |
| 時間 | **2026.06.20 ~ 2026.09.01**（約 10 週 / 73 天） |
| 定位 | 海風第一個季度活動，暑假限定 |
| 參加 | **免費**，所有玩家自動參加 |
| 季卡加碼 | 季卡玩家享額外獎勵（活動額外＋季節額外） |
...
| 階段 | 時間 | 內容 |
|---|---|---|
| 🌅 開幕期 | 6/20 ~ 7/04 | 季節開跑、開幕活動、新手引導任務 |
| 🌊 正式期 | 7/04 ~ 8/17 | 每週任務、階段性內容解鎖、季節活動 |
| 🏁 結算期 | 8/17 ~ 9/01 | 最終挑戰、獎勵兌換截止、結算排名 |
...
| 類型 | 說明 | 頻率 |
|---|---|---|
| 🌟 季節主線 | 跟季節主題相關的大型任務鏈 | 整季（分階段解鎖） |
| 📋 每週任務 | 每週更新 3 個任務，完成獲得季節代幣 | 每週重置 |
| 🎯 每日挑戰 | 簡單的日常小任務 | 每日重置 |
...
- [x] 季節框架規劃（三階段時程 + 代幣系統）
- [x] 每日任務池（20 組任務輪替）
- [x] 每週任務池（7 週 × 3 個 = 21 個任務）
- [x] 主線任務內容（4 章：風起/相遇/共築/風緣）
- [x] 最終挑戰設計（5 個綜合任務）
- [x] 風緣碎片經濟（產出量、商店物價、通膨控制）
- [x] 季節商店商品清單（免費 12 項 + 季卡 6 項）
- [x] 季節稱號設計（6 個：白/藍/綠/紅/紫/金）
- [x] NPC 設定（風緣使者對話腳本）
- [x] 分層道具系統（粒子 3 Tier，T3 季節結束絕版）
- [x] 風之羽掉落機制
- [x] 大廳裝飾規劃
...
- [ ] 網站頁面（風緣季專屬頁面、任務進度、商店頁面）
- [ ] NPC 皮膚製作 + Citizens 實裝
- [ ] 粒子效果實作（MMOItems + MythicMobs）
- [ ] 家具道具實作（MMOItems）
