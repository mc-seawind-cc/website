# 海風 SeaWind 網站設計文件
| 2026.05.14 | 海風網站助手 | **贊助頁季卡定位重調整＋Hero 潤飾＋移除資金用途＋移除贈卡**：①季卡卡片：從「9 項功能，到期自動關閉」改為主打「活動＋季節額外獎勵」，隨地開啟降為附贈功能；②會員卡片：「22 項永久回饋」→「VIP 永久身份」；③組合卡片：「一次解鎖全部 31 項回饋」→「一次解鎖 VIP 永久身份＋當季活動加成」；④對比表：季卡專屬區從底部移至待設計前（更顯眼），隨地開啟分類標籤從「會員永久/季卡到期關閉」→「兩個方案都有」；⑤FAQ 重寫——會員vs季卡差別改為「長期投資 vs 活動季加成」、季卡到期答案補充活動/季節獎勵也會關閉、組合方案改用 VIP 術語；⑥meta/og description 同步更新；⑦Hero 文案潤飾——從三行長句精簡為兩行；⑧移除資金用途區塊（Hero 已涵蓋）；⑨移除贈卡區塊＋贈卡FAQ（贈卡系統暫緩）。cache busting → 140514f。頁尾日期 → 2026.05.14 |
...
| 項目 | 決定 |
|---|---|
| 會員 | **800 冷媒**（永久），8 項數值擴充 ＋ 7 項榮譽標識 |
| 季卡 | **400 冷媒/季**（約 40-80 天），13 項便利功能（到期關閉，無會員稱號） |
| 組合 | **1000 冷媒** | 會員永久 ＋ 送 1 季季卡（省 200） |
| 贈卡 | **30 風汐錠**，送朋友 7 天季卡功能試用 |
...
- **會員（24 項）**：數值擴充 8 ＋ 隨地開啟 7 ＋ 便利功能 4 ＋ 榮譽標識 3 ＋ 待設計 2，永久保留
- **季卡專屬（2 項）**：活動額外獎勵 ＋ 季節額外獎勵，每季更新

### 待辦

- [ ] 贈卡技術實作（序號系統 + LuckPerms）
- [ ] 季度系統設計（概念階段）
- [ ] 多功能工具（暫緩）
