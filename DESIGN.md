...
| 項目 | 決定 | 狀態 |
|---|---|---|
| 每日冷媒 | 會員每日簽到 +2 冷媒（月回饋 60 冷媒＝30%） | 待實作 |
| 風汐錠用途 | 改為簽到基礎獎勵（會員/非會員都有），用來買贈卡、工具包等 | 待設計商品 |
| 限定稱號 | 季度制（每季換一次），稱號綁定粒子效果 | 待設計稱號 |
| 贈卡 | 用風汐錠購買，送朋友 7 天會員體驗 | 待定價 |
...
#### 產出
- 會員簽到：每日 2 冷媒 + 風汐錠（數量待定）
- 非會員簽到：每日風汐錠（數量待定）
- 目標：會員月累積約 60 顆，非會員約 30 顆

#### 消耗：外觀類

| 商品 | 價格 | 說明 | 狀態 |
|---|---|---|---|
| 稱號顏色自訂 | 10 顆 | 自訂遊戲內稱號的顏色 | 待確認 |
| 季度限定稱號（含粒子） | 30 顆 | 過期的季度稱號可回購 | 待確認 |
| 自訂加入訊息 | 25 顆 | 進入伺服器時顯示的歡迎文字 | 待確認 |

#### 消耗：功能類

| 商品 | 價格 | 說明 | 狀態 |
|---|---|---|---|
| 額外個人傳送點 ×1 | 15 顆 | 永久多一個傳送點額度 | 待確認 |
| 額外倉庫格 ×4 | 20 顆 | 永久多 4 格倉庫空間 | 待確認 |
| 領地改名卡 | 10 顆 | 更改領地顯示名稱 | 待確認 |
| 個人傳送點改名卡 | 5 顆 | 更改傳送點名稱 | 待確認 |

#### 消耗：贈卡與工具包

| 商品 | 價格 | 說明 | 狀態 |
|---|---|---|---|
| 贈卡（7 天體驗） | 待定 | 送給非會員朋友，7 天會員體驗 | 待定價 |
| 工具包 | 待定 | 內容待設計 | 待設計 |
...
| 商品 | 價格 | 說明 | 狀態 |
|---|---|---|---|
| 隨機傳送卷 | 3 顆 | 隨機傳送到主世界某個位置 | 待確認 |
| 領地回家卷 | 2 顆 | 立即傳送回主領地 | 待確認 |
| 經驗加成（30 分鐘） | 5 顆 | 獲得經驗值 ×1.5 倍 | 待確認 |
...
一個月累積 → 60 冷媒 + ? 風汐錠
                ...
                v
冷媒 → 會員月費回饋（感覺回來了）
風汐錠 → 風汐商店 → 贈卡/工具包/外觀/功能
```

### 季度稱號規劃

| 季度 | 月份 | 主題方向 | 稱號（待取） | 粒子效果 |
|---|---|---|---|---|
| Q2 2026 | 5-7 月 | 海洋/守護 | 待取 | 海浪/泡泡 |
| Q3 2026 | 8-10 月 | 夏日/冒險 | 待取 | 陽光/沙灘 |
| Q4 2026 | 11-1 月 | 冬季/溫暖 | 待取 | 雪花/極光 |
| Q1 2027 | 2-4 月 | 春天/新生 | 待取 | 花瓣/新芽 |
