...
| 2026-04-18 | 海風網站助手 | 🧋 珍奶日活動頁全面視覺升級：(1) Hero 區新增 CSS 珍奶杯插畫（漸層奶茶+珍珠+吸管+杯蓋+高光）(2) 新增快速統計列（13 飲品/14 原料/35 天/4 階段）(3) 新增「玩法總覽」4 步驟引導卡片（領任務→收集原料→兌換珍奶→解鎖獎勵）(4) 時程從文字列表改為視覺化節點連線圖（圓形節點+虛線連接+日期+說明）(5) 新增原料總覽視覺化網格（14 種 emoji+名稱）(6) 飲品配方從表格改為響應式卡片（名稱+代幣+配方+效果+冷卻，金色特調特殊樣式）(7) 社群挑戰從 task-item 改為分色挑戰卡片（探索藍/社群綠/建築金）(8) 代幣商店從平行列改為階梯式列表（價格+圖標+名稱，高階項目 premium 樣式）(9) 整體新增 event-block-title 圖標裝飾、區塊分隔線、完整 light 模式+響應式+列印樣式 |
...
| 2026-04-18 | 海風網站助手 | 🧋 活動時程調整：刪除預告期（4/27–28），暖身期提前至 4/27 開始。4 階段：測試→暖身(4/27–5/3)→正式(5/4–27)→收尾(5/28–31) |
| 2026-04-18 | 海風網站助手 | 🧋 每日簽到從「贊助者限定」改為「會員限定」：所有會員皆可領取，贊助者福利表移除簽到項目 |
