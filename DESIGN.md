...
| 產出方式 | 會員簽到每日 2 顆（月 60 顆） | ✅ |
| 非會員取得 | 目前無。未來考慮開放其他管道（任務？活動？） | ⏳ 待設計 |
| 粒子效果 | 跟稱號綁定，不是獨立商品 | ✅ |
...
| 商品 | 概念 | 狀態 |
|---|---|---|
| 贈卡（7 天會員體驗） | A 購買 → 生成序號 → B 兌換 → LuckPerms 加 7 天會員群組 → 自動到期 | 🔧 待設計（技術：LuckPerms timed permissions） |
| 多功能工具 | 一把工具多模式切換附魔（如鎬：效率/絲綢/幸運，右鍵循環） | 🔧 待設計（技術：需確認插件支援） |
| 季度限定稱號 | 每季一個主題稱號＋綁定粒子效果，當季可購買，過季等明年返場 | 🔧 待設計（見下方季度系統） |
| 季度限定物品 | 每季 2-3 個自訂物品，當季可買，過季絕版 | 🔧 待設計 |

### 季度系統（概念階段）

> 參考光遇的季節系統：每季有主題、有專屬物品、有時間限制。
> 季節長度約 40-80 天，季與季之間有空窗期。
...
一個月累積 → 60 冷媒 + 60 風汐錠
                ...
                v
冷媒 → 會員月費回饋感（每天都有東西領）
風汐錠 → 風汐商店 → 已確認商品(5~25) / 贈卡(待設計) / 季度商品(待設計)
```
...
- [x] 確認產出方式 → 會員每日 2 顆
- [x] 設計已確認商品（8 項）
- [ ] 設計贈卡系統（LuckPerms 技術確認）
- [ ] 設計多功能工具（插件確認）
- [ ] 設計季度系統完整框架（季節長度、空窗期、內容規劃）
- [ ] 設計第一季內容（名稱、稱號、物品、粒子效果）
- [ ] 確認非會員是否開放風汐錠取得
- [ ] 設計風汐商店 UI
- [ ] 更新會員頁面文案
...
## 10. 修改記錄
| 2026.05.13 | 海風網站助手 | **DESIGN.md 8d 節重寫**：①風汐商店系統設計重新整理——已確認商品 8 項（稱號顏色/傳送點/倉庫/改名卡/加入訊息/傳送卷/回家卷）、待設計商品 4 項（贈卡/多功能工具/季度稱號/季度物品）；②贈卡概念確認（LuckPerms timed permissions）但細節待設計；③季度系統從「定稿」改為「概念階段」——季節 40-80 天＋空窗期＋每季專屬內容＋過季等明年返場；④移除之前未確認的設計（非會員產量/工具包具體內容/經驗加成）；⑤新增風汐錠物品規格（MMOItems config）。頁尾日期 → 2026.05.13 |
