...
- [ ] 季節主線任務內容（跟「風緣」主題相關的故事/挑戰）
- [ ] 每週任務池（需要多少組任務、輪替規則）
- [ ] 每日挑戰任務池
- [ ] 風緣碎片經濟（產出量、商店物價、通膨控制）
- [ ] 季節商店商品清單（免費可兌換 vs 季卡專屬）
- [ ] 季節稱號設計（名稱、取得條件、樣式差異）
- [ ] 活動設計（開幕活動、中間活動、結算活動）
...
> 「風」是海風的風，「緣」是緣分的緣。
> 在海風相遇就是一種緣分，風緣季是這段緣分的紀念。

可能的方向：
- 風 + 緣分 → 社群互動類任務（跟其他玩家合作、交換、拜訪）
- 風 + 探索 → 探索類任務（探索特定地標、發現隱藏地點）
- 風 + 建造 → 建築類任務（建造特定風格的建築）
- 緣 + 收集 → 收集類任務（收集特定物品、完成圖鑑）
